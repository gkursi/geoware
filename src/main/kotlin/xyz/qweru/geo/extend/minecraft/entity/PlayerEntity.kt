...
fun Player.inRange(range: Float) =
    this.distanceToSqr(mc.player?.eyePosition ?: Vec3.ZERO) <= Mth.square(range)

fun Player.inRange(range: ClosedRange<Float>) =
    this.distanceToSqr(mc.thePlayer).let {
        it >= Mth.square(range.start) && it <= Mth.square(range.endInclusive)
    }

fun Player.inFov(fov: Float) =
    RotationHelper.getAngle(this) <= fov

fun Player.visiblePoint() =
    WorldHelper.blockCollision(
        this.level(),
        mc.player?.eyePosition ?: Vec3.ZERO,
        this.boundingBox
    )
