...
class ModuleAimAssist : Module("AimAssist", "Auto aim", Category.COMBAT) {
    ...
    val hSpeed by sg.int("H Speed", "Horizontal camera speed", 30, 0, 360)
    val vSpeed by sg.int("V Speed", "Horizontal camera speed", 0, 0, 360)
    val random by sg.longRange("Random", "Randomization", -40L..40L, -100L..100L)
    ...
    val disableRange by sg.float("Disable Range", "Won't aim assist when this close to an enemy", 0.3f, 0f, 3f)
    ...
    private fun onFrame(e: GameRenderEvent) {
        ...
        if (target == null || !lock || !target!!.inRange(range) || !target!!.isAlive) target =
            TargetHelper.findTarget(range, fov, invisible)
        if (target == null || !target!!.inFov(fov) || target!!.inRange(disableRange)) return
